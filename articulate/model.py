...
class ParametricModel:
    ...
    def __init__(self, official_model_file: str, use_pose_blendshape=False, device=torch.device('cpu')):
        ...
        def _to_np(v):
            """Extract a plain float32 ndarray from a value that may be a chumpy array."""
            # Check for chumpy-like object by type name (avoid calling .r which may be an array)
            type_name = type(v).__name__
            if type_name == '_Ch' or (type_name not in ('ndarray',) and hasattr(v, '__setstate__')):
                r = getattr(v, '_r', None)
                if r is None:
                    r = getattr(v, 'r', None)
                if r is not None:
                    return np.array(r, dtype=np.float32)
            if hasattr(v, 'toarray'):  # sparse matrix
                return np.array(v.toarray(), dtype=np.float32)
            return np.array(v, dtype=np.float32)

        self._J_regressor = torch.from_numpy(_to_np(data['J_regressor'])).to(device)
        self._skinning_weights = torch.from_numpy(_to_np(data['weights'])).to(device)
        self._posedirs = torch.from_numpy(_to_np(data['posedirs'])).to(device)
        self._shapedirs = torch.from_numpy(_to_np(data['shapedirs'])).to(device)
        self._v_template = torch.from_numpy(_to_np(data['v_template'])).to(device)
        self._J = torch.from_numpy(_to_np(data['J'])).to(device)
        self.face = data['f']
    ...
    def view_mesh(self, vertex_list: list, fps=60, colors=None, distance_between_subjects=0.8):
        r"""
        ...
        faces = np.concatenate(f_list)
        if verts.shape[0] > 1:
            vo3d.render_sequence_3d(verts, faces, 720, 720, 'a.mp4', fps, colors=colors, visible=True)
        else:
            vo3d.vis_mesh(verts[0], faces)

    def view_motion(self, pose_list: list, tran_list: list = None, fps=60, colors=None, distance_between_subjects=0.8):
        r"""
